...
```js
import Vue from 'vue';
import { PullRefresh } from '@apeiwan/pi-play';
...
```html
<pi-play-pull-refresh v-model="isLoading" @refresh="onRefresh">
  <p>刷新次数: {{ count }}</p>
...
```html
<pi-play-pull-refresh v-model="isLoading" :head-height="80" @refresh="onRefresh">
...
```html
<pi-play-pull-refresh style="min-height: 100vh;" />
```
