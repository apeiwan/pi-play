...
```js
import Vue from 'vue';
import { List } from '@apeiwan/pi-play';
...
```html
<pi-play-pull-refresh v-model="refreshing" @refresh="onRefresh">
  <pi-play-list
    v-model="loading"
    :finished="finished"
    finished-text="没有更多了"
...
若 List 的内容使用了 float 布局，可以在容器上添加`pi-clearfix`类名来清除浮动，使得 List 能正确判断元素位置
