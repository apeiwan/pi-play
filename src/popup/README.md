...
```js
import Vue from 'vue';
import { Popup } from '@apeiwan/pi-play';
...
```html
<pi-play-popup v-model="show">内容</pi-play-popup>
```
...
```html
<pi-play-popup v-model="show" position="top" :style="{ height: '30%' }" />
```
...
```html
<pi-play-popup v-model="show" round position="bottom" :style="{ height: '30%' }" />
```
...
```html
<!-- 挂载到 body 节点下 -->
<pi-play-popup v-model="show" get-container="body" />

<!-- 挂载到 #app 节点下 -->
<pi-play-popup v-model="show" get-container="#app" />

<!-- 通过函数指定挂载位置 -->
<pi-play-popup v-model="show" :get-container="getContainer" />
```
