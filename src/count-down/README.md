...
```js
import Vue from 'vue';
import { CountDown } from '@apeiwan/pi-play';
...
```html
<pi-play-count-down :time="time" format="DD 天 HH 时 mm 分 ss 秒" />
```
...
```html
<pi-play-count-down millisecond :time="time" format="HH:mm:ss:SS" />
```
