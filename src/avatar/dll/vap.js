...
(function (global, factory) {
  ...
  const arrayLikeToArray = _arrayLikeToArray;
  ...
  const unsupportedIterableToArray = _unsupportedIterableToArray;
  ...
  const D__project_vapSource_web_node_modules__babel_runtime_regenerator = runtime_1;
  ...
  const superPropBase = _superPropBase;

  const get = createCommonjsModule(function (module) {
  ...
  const getPrototypeOf$1 = createCommonjsModule(function (module) {
    function _getPrototypeOf(o) {
      module.exports = _getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf : function _getPrototypeOf(o) {
        return o.__proto__ || Object.getPrototypeOf(o);
      };
  ...
  ***************************************************************************** */
  ...
  const FrameParser = /* #__PURE__ */function () {
    ...
    createClass(FrameParser, [{
      ...
      value: function makeTextImg(_ref) {
        ...
        const font = [''.concat(fontSize, 'px'), 'Arial'];
  ...
  const VapVideo = /* #__PURE__ */function () {
    ...
    createClass(VapVideo, [{
      ...
      value: function initVideo() {
        ...
        window.enableInlineVideo && window.enableInlineVideo(video);
        if (options.precache) {
  ...
  function _createSuper(Derived) { const hasNativeReflectConstruct = _isNativeReflectConstruct(); return function _createSuperInternal() { const Super = getPrototypeOf$1(Derived); let result; if (hasNativeReflectConstruct) { const NewTarget = getPrototypeOf$1(this).constructor; result = Reflect.construct(Super, arguments, NewTarget) } else { result = Super.apply(this, arguments) } return possibleConstructorReturn(this, result) } }
  ...
  const WebglRenderVap = /* #__PURE__ */function (_VapVideo) {
    inherits(WebglRenderVap, _VapVideo);

    const _super = _createSuper(WebglRenderVap);
    ...
    createClass(WebglRenderVap, [{
      ...
      value: function setCanvas() {
        ...
        const _this$options = this.options;
          const {width} = _this$options;
          const {height} = _this$options;
      ...
      value: function initWebGL() {
        ...
        const _this$instance = this.instance;
          let {gl} = _this$instance;
          let {vertexShader} = _this$instance;
          let {fragmentShader} = _this$instance;
          let {program} = _this$instance;
      ...
      value: function initFragmentShader() {
        ...
        const textureSize = gl.getParameter(gl.MAX_TEXTURE_IMAGE_UNITS) - 1; // const textureSize =0
      ...
      value: function initTexture() {
        ...
        for (const key in resources) {
          ...
          const _sampler = gl.getUniformLocation(this.program, 'u_image'.concat(i));
      ...
      value: function initVideoTexture() {
        ...
        const {info} = this.vapFrameParser.config;
        const ver = [];
        const vW = info.videoW;
          const vH = info.videoH;

        const _info$rgbFrame = slicedToArray(info.rgbFrame, 4);
          const rgbX = _info$rgbFrame[0];
          const rgbY = _info$rgbFrame[1];
          const rgbW = _info$rgbFrame[2];
          const rgbH = _info$rgbFrame[3];

        const _info$aFrame = slicedToArray(info.aFrame, 4);
          const aX = _info$aFrame[0];
          const aY = _info$aFrame[1];
          const aW = _info$aFrame[2];
          const aH = _info$aFrame[3];
        ...
        if (this.vapFrameParser) {
          ...
          if (frameData && frameData.obj) {
            frameData.obj.forEach(function (frame, index) {
              ...
              const {info} = _this2.vapFrameParser.config;
              const vW = info.videoW;
                const vH = info.videoH; // frame坐标是最终展示坐标，这里glsl中计算使用视频坐标

              const _info$rgbFrame2 = slicedToArray(info.rgbFrame, 2);
                const rgbX = _info$rgbFrame2[0];
                const rgbY = _info$rgbFrame2[1];

              const _frame$frame = slicedToArray(frame.frame, 4);
                const x = _frame$frame[0];
                const y = _frame$frame[1];
                const w = _frame$frame[2];
                const h = _frame$frame[3];

              const _frame$mFrame = slicedToArray(frame.mFrame, 4);
                const mX = _frame$mFrame[0];
                const mY = _frame$mFrame[1];
                const mW = _frame$mFrame[2];
                const mH = _frame$mFrame[3];
      ...
      key: 'destroy',
      value: function destroy() {
        const _this$instance2 = this.instance;
          const {canvas} = _this$instance2;
          const {gl} = _this$instance2;
